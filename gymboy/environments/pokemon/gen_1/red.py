...
class PokemonRedFullImage(PokemonRed):
    ...
    def observation(self) -> dict[str, Any]:
        ids = _pokemon_ids(self.pyboy, yellow=False).astype(np.float32)
        team_size = np.array([_team_size(self.pyboy, yellow=False)]).astype(np.float32)
        exps = _exps(self.pyboy, yellow=False).astype(np.float32)
        levels = _levels(self.pyboy, yellow=False).astype(np.float32)
        max_hps = _max_hps(self.pyboy, yellow=False).astype(np.float32)
        hps = _hps(self.pyboy, yellow=False).astype(np.float32)
        moves = _moves(self.pyboy, yellow=False).astype(np.float32)
        max_pps = _max_pps(self.pyboy, yellow=False).astype(np.float32)
        pps = _pps(self.pyboy, yellow=False).astype(np.float32)
        badges = np.array([_badges(self.pyboy, yellow=False)]).astype(np.float32)
        money = np.array([_money(self.pyboy, yellow=False)]).astype(np.float32)
        img = (
...
class PokemonRedMinimalImage(PokemonRed):
    ...
    def observation(self) -> dict[str, Any]:
        ...
        badges = np.array([_badges(self.pyboy, yellow=False)]).astype(np.float32)
        money = np.array([_money(self.pyboy, yellow=False)]).astype(np.float32)
        img = _game_area(self.pyboy, yellow=False).astype(np.float32)
        return {
            "pokemon": {
                "ids": ids,
                "team_size": team_size,
